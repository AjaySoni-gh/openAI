...
const renderEventMessage =(event:Frame)=>{
    switch(event.type){
        ...
        case "callChat": return(
            <div>Chat in progress with your input{">>"} {String(event.input)}</div>
        )    
        case "callProgress":
            return null;
        case "callFinish":return(
            <div>Call finished:{" "}
  ...
  return <div>Run finished: {event.end}</div>;
            case "callSubCalls":
                return (
                    <div>
                  ...
                  default: return <pre>{JSON.stringify(event,null,2)}</pre>
